...
`wrap-datastar` that `:biff.datastar/epoch` has changed.

### :biff.datastar/epoch

Atom containing an int. This value is incremented by `refresh` and is used by
`wrap-datastar` to infer when another SSE push should be sent to connected
clients.
...
pushes from `wrap-datastar` for a single connection. Default 15 (~67 pushes per
second).
...
load and set on incoming Ring requests by `wrap-datastar`. May be used to store
tab-specific state on the backend.
