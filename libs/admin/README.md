...
```clojure
com.biffweb/admin {:mvn/version "2.0.0-rc20"}
```
...
Add `module` and `component` to your biff.core modules/components:
...
The metrics page displays up to 30 days of data, so `get-usage-events` and
`get-revenue-events` don't need to include events older than that. `get-users`
is used both to populate the metrics page (with # new users per day) and the
users page (with a paginated list of all users), so it should return everything.
