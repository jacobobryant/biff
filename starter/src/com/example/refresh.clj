...
(defn wrap-htmx-refresh [handler]
  (fn [{:keys [biff.refresh/enabled
               biff.refresh/insert-fn
               uri]
        :or {replace-str "</body"}
        :as ctx}]
    (cond
      (not enabled) (handler ctx)
      (= uri "__biff/refresh") (ws-handler ctx)
      )
    )
  )

(defn use-htmx-refresh [{:keys [biff/handler] :as ctx}]
  (let [handler (wrap-htmx-refresh handler)]
    (merge ctx {:biff/handler handler})))
